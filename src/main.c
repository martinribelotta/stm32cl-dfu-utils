...
skip_setup:
	...
	case MODE_UPLOAD:
		if (dfuse) {
		    if (dfuse_address)
		        dfuse_set_address_pointer(dif->dev_handle,
				  dif->interface, dfuse_address, 0);
		    if (dfuse_do_upload(dif->dev_handle, dif->interface,
	...
	case MODE_DOWNLOAD:
		if (dfuse) {
		    if (dfuse_address) {
		        if (dfuse_do_bin_dnload(dif->dev_handle, dif->interface,
				  transfer_size, filename, dfuse_address) < 0)
			    exit(1);
		    } else {
			if (dfuse_do_dfuse_dnload(dif->dev_handle, dif->interface,
				  transfer_size, filename) < 0)
			exit(1);
		    }
		} else {
